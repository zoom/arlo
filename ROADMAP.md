...
- [ ] **Home dashboard automation** — Basic highlights/reminders endpoints exist, but the weekly digest, recurring topics, action-item rollups, and several vertical dashboard sections remain demo-oriented.
- [x] **Multi-source search** — Search queries meeting titles, AI summary JSON, and transcripts with MySQL-compatible `LIKE` queries. Results are prioritized by source (titles first, summaries second, transcripts third) with section labels when multiple categories match. The AppShell dropdown shows type badges and limits to 5 results.
...
- [x] **AI-generated meeting titles** — Sparkle icon in `MeetingDetailView` calls `POST /api/ai/generate-title` to generate a concise title from transcript/summary. Generated title pre-fills the inline editor for review before saving.
...
- [x] **Guest mode showcase** — Guest-mode screens, invitation UI, presence UI, and SDK context handling are implemented as a design/reference showcase. The current production backend requires an authenticated signed WebSocket token, so the old anonymous/null-token transcript behavior must not be described as production access. Manifest: added guest mode and invitation APIs.
...
- [x] **Rewrite TROUBLESHOOTING.md** — Replaced the pre-v1.0 troubleshooting notes with the current OAuth, WebSocket, RTMS, Docker, AWS worker, AI, and SDK capability guidance in [`docs/TROUBLESHOOTING.md`](./docs/TROUBLESHOOTING.md).
- [x] **Update ARCHITECTURE.md** — Replaced the obsolete architecture with the current CRA, MySQL, Valkey, WebSocket, RTMS worker, and AWS Terraform design in [`docs/ARCHITECTURE.md`](./docs/ARCHITECTURE.md).

### README — Setup and Configuration
- [x] Document the exact SDK capabilities that need to be enabled in the Zoom Marketplace app configuration
- [x] Explain RTMS scope requirements and the optional auto-open scopes
- [x] Note that Guest Mode and In-Client OAuth must both be enabled in the Zoom Marketplace app settings when those surfaces are used
- [x] Document that `appssdk.zoom.us` must be added to the domain allowlist
- [x] Add required OAuth scopes to Step 6 (manual Zoom app config)
- [x] Expand environment variables documentation through `.env.example` and the fresh-install checklist
- [x] **Clarify local development** — Docker Compose is the primary supported path; the current schema uses MySQL and `prisma db push`, documented in [`docs/FRESH_INSTALL_CHECKLIST.md`](./docs/FRESH_INSTALL_CHECKLIST.md).
...
### README — General
- [x] Update the Documentation section to include ROADMAP.md
